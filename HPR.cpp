        ...
        SimpleArray< double, 14 >& N )
{
    ...
    if( dir == 0 )
    {
        N[4] = u[1] * ( A_L(0, 1) - A_R(0, 1) ) + u[2] * ( A_L(0, 2) - A_R(0, 2) );
        N[5] = u[1] * ( A_L(1, 1) - A_R(1, 1) ) + u[2] * ( A_L(1, 2) - A_R(1, 2) );
        N[6] = u[1] * ( A_L(2, 1) - A_R(2, 1) ) + u[2] * ( A_L(2, 2) - A_R(2, 2) );

        N[7] = - u[0] * ( A_L(0, 1) - A_R(0, 1) ); 
        N[8] = - u[0] * ( A_L(1, 1) - A_R(1, 1) ); 
        N[9] = - u[0] * ( A_L(2, 1) - A_R(2, 1) ); 
        
        N[10] = - u[0] * ( A_L(0, 2) - A_R(0, 2) ); 
        N[11] = - u[0] * ( A_L(1, 2) - A_R(1, 2) ); 
        N[12] = - u[0] * ( A_L(2, 2) - A_R(2, 2) ); 
    }
    else if( dir == 1 )
    {
    ...
    for( int i = nGhostCells; i < nGhostCells + nCellsX; i++ )
    {
        for( int j = nGhostCells; j < nGhostCells + nCellsY; j++ )
        {
            ...
            for( int k = 0; k < 3; k++ )
            {
                for( int l = 0; l < 3; l++ )
                {
                    consVars[cell][4 + 3 * k + l] += 0.25 * curlcurlA(k, l) / 5; 
                }
            }
        }
    }
